...
```{r comment="Daily total steps: "}
summary(daily_data$total_steps)
```

```{r comment="Mean daily total steps: "}
mean_daily_steps <- mean(daily_data$total_steps)
mean_daily_steps
```

```{r comment="Median daily total steps: "}
...
Solid red vertical line indicates the mean (`r mean_daily_steps`), dashed red vertical line indicates the median (`r median_daily_steps`).  
...
However, visual inspection indicates fat right tails for many intervals:  there are `r nrow(cc_data[cc_data$steps > 500, ])` observations of more than 500 steps.  (500 steps in 5 minutes is `r 500/5` steps per minute or `r 500/(5*60)` steps per second.  180 steps per minute, or `r 180/60` steps per second, is considered an appropriate running pace for serious runners.)  At the same time, the number of steps is bounded on the left at 0.  Since the mean can be a misleadingly high measure of central tendency with these kinds of distributions, we plot every observation in the dataset, and look for the central tendency using the mean, median, and a GAM estimate.  
...
Observations with steps > `r ylim[2]` not shown here but used in plot calculations.  Yellow curve:  GAM estimate with 95% confidence interval.  Red solid curve: mean.  Red dashed curve: median. 
...
1. In the full dataset, there are `r sum(complete.cases(readdata))` complete cases and `r sum(!complete.cases(readdata))` incomplete cases (`r sum(!complete.cases(readdata)) / nrow(readdata) * 100`%).  The analysis so far as used only the complete cases.  
...
```{r comment='Days with missing data, by day of the week'}
table(weekdays(missing_data))
```
```{r comment='Days of the study, by day of the week'}
...
Calculate mean and median. In the following six displays, we first display the non-imputed statistics, then the statistics with imputed data.  
...
The median is unchanged, which is not surprising.  The mean is slightly higher with the imputed data (150 steps).  
...
1. To add a new factor column, we first construct a function that tells us whether a given date was on the weekend. It's then simple to build the column using `ifelse`.  
...
2. We take the same approach as in "What is the average daily activity pattern?," with the addition of the "weekday" and "weekend" factors.  
...
# A facet_wrap separates everything by the weekend factor
